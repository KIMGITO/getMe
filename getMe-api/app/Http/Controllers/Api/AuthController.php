...
class AuthController extends Controller
{
    ...
    public function loginInit(Request $request)
    {
        ...
        //  Admins/Riders (Password or PIN based)
        return response()->json([
    ...
    public function loginVerify(Request $request, RiderService $riderService, ClientService $clientService)
    {
        ...
        if ($user->role === 'client') {
            ...
            // verify pin or password for rider or admin
        ...
        // log out other riders to have only one device logged in
        if ($user->role == 'rider') {
            $user->tokens()->delete();
        }
        ...
        if ($user->role == 'client') {
            $isProfileSet = $clientService->isClientProfileSet($user);
        }
